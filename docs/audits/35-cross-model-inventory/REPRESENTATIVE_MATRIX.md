...
- Runtime root: `UPSTREAM_PROVIDER_INSTABILITY_CONFIRMED` (2× `finish=null` truncations; same `cheaperinference` label as successes)
- 1610-char stop: `VALID_MODEL_EARLY_STOP` (separate from transport)
- Rhythm on valid n=6: `RHYTHM_REGRESSION_SYSTEMIC`
- Live matrix still blocked until transport reliability + functional reconfirm
