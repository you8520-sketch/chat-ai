...
- Hybrid server compositing emits text glyphs only. Strategy A uses shared planned
  anchors. Strategy B uses deterministic Sharp pixel analysis of enclosed bright
  regions and rejects ambiguous matches without drawing a new body.
...
signature, attempt/result metadata, safety categories, provider request id, and
usage evidence. Prompt text, source prose, reference URLs, and reference bytes are
not included.

## Human QA gate

Compare a normal production-overlay result with a blank-balloon hybrid result from
the same safe source scene. Review panel composition, shot variety, identity,
scene fidelity, facial reaction, balloon naturalness, subject occlusion, dialogue
readability, speaker attribution, and manga likeness. The implementation does not
self-score those dimensions.
