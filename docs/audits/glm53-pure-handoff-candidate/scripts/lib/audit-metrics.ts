...
export function attributeDialogueInResponse(input: {
  ...
  SOURCE_USER_QUOTED_DIALOGUE_COUNT: number;
  ...
  return {
    ...
    SOURCE_USER_QUOTED_DIALOGUE_COUNT:
      by_attribution.SOURCE_USER_QUOTED_DIALOGUE.length,
...
/** Mechanical T2 replay detector for already-resolved T2 interaction topics. */
export function detectT2ReplayCandidate(input: {
  responseText: string;
  t2AssistantVisible: string;
  t2UserRaw: string;
}) {
  const t = String(input.responseText || "");
  const topics = [
    {
      id: "WHY_LOOKING",
      label: "why he was looking",
      t2_markers: [/왜\s*쳐다/, /몰라서\s*물어/, /눈을\s*돌려/],
      response_markers: [/왜\s*쳐다/, /눈을\s*돌려/, /쳐다봤/],
    },
    {
      id: "FOOD_HUNGER",
      label: "food/hunger conversation",
      t2_markers: [/배고프/, /밥\s*먹/, /에피타이저/],
      response_markers: [/배고프/, /밥\s*먹/, /에피타이저/, /밥이고\s*뭐고/],
    },
    {
      id: "FIRST_KISS",
      label: "first kiss interpretation",
      t2_markers: [/입술/, /키스|쪽,|마찰/, /부딪쳐\s*오/],
      response_markers: [/입술/, /키스|쪽,|마찰/, /먼저\s*입술/, /에피타이저/],
    },
  ] as const;
  ...
  for (const topic of topics) {
    const t2Text = `${input.t2AssistantVisible}\n${input.t2UserRaw}`;
    const t2Present = topic.t2_markers.some((re) => re.test(t2Text));
    const responsePresent = topic.response_markers.some((re) => re.test(t));
    hits.push({
      topic_id: topic.id,
      topic_label: topic.label,
      t2_present: t2Present,
      response_present: responsePresent,
      replay_candidate: t2Present && responsePresent,
    });
  }

  const replayTopics = hits.filter((h) => h.replay_candidate).map((h) => h.topic_id);
  return {
    T2_REPLAY_CANDIDATE: replayTopics.length > 0,
    replay_topic_ids: replayTopics,
    topic_hits: hits,
  };
}
