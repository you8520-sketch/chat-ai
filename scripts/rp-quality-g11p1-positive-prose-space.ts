/**
 * Phase G11-P1 — Positive Prose Space Replacement.
 *
 * FREEZE: I1/D3 CURRENT USER TURN tail (length owner + dynamic dialogue terminal).
 * FREEZE: Scene Pacing / communication / dialogue budget resolvers BYTE_IDENTICAL.
 * SOLE VARIABLE: REPLACE generic [IMMERSIVE PROSE] with [IMMERSIVE LONGFORM PROSE].
 * Arm W (L1 combined terminal) FORBIDDEN.
 *
 * PRODUCTION WIRE = 0 · MERGE = 0 · retry/continuation/repair = 0
 *
 *   PHASE=preaudit|live STAGE=1|2 FIXTURES=B,D,F DRAWS=2 \
 *     node --conditions=react-server --import tsx \
 *     scripts/rp-quality-g11p1-positive-prose-space.ts
 */
...
import {
  ...
  IMMERSIVE_LONGFORM_PROSE_MARKER,
  LEGACY_IMMERSIVE_PROSE_BLOCK,
  LEGACY_IMMERSIVE_PROSE_MARKER,
...
const OUT_ROOT =
  ...
  "/opt/cursor/artifacts/rp-quality-g11p1-positive-prose";
const DOCS =
  process.env.DOCS_DIR ?? "docs/audits/rp-positive-prose-g11p1";
const FIX_PATH =
...
const L1_COMBINED_MARKER_RE = /\[이번 응답\](?! 대화)/;
...
async function runPreaudit(modelId: string) {
  // API=0 always validates full A–F matrix.
  const allFixtures = (
  ...
  const promptSize = {
    legacy_immersive_chars: LEGACY_IMMERSIVE_PROSE_BLOCK.length,
    positive_longform_chars: IMMERSIVE_LONGFORM_PROSE_BLOCK.length,
    delta_chars:
      IMMERSIVE_LONGFORM_PROSE_BLOCK.length - LEGACY_IMMERSIVE_PROSE_BLOCK.length,
    legacy_immersive_tokens_est: Math.ceil(
      LEGACY_IMMERSIVE_PROSE_BLOCK.length / 3.5
    ),
    positive_longform_tokens_est: Math.ceil(
      IMMERSIVE_LONGFORM_PROSE_BLOCK.length / 3.5
    ),
  };
  ...
  const checks = allFixtures.map((f) => {
    ...
    const positiveProse = (
      ass.systemText.match(/\[IMMERSIVE LONGFORM PROSE\]/g) ?? []
    ).length;
    const legacyProse = (
      ass.systemText.match(/\[IMMERSIVE PROSE\]/g) ?? []
    ).length;
    const l1Combined = L1_COMBINED_MARKER_RE.test(ass.lastUserContent)
      ? 1
      : 0;
    return {
      id: f.id,
      ...
      length_owner: ass.lastUserContent.includes(USER_TAIL_LENGTH_OWNER_SENTENCE)
        ? 1
        : 0,
      l1_combined_terminal: l1Combined,
      scene_pacing: ass.owners.scene_pacing,
      scene_flow: ass.owners.scene_flow,
      positive_prose_owner: positiveProse,
      legacy_prose_owner: legacyProse,
      positive_block_embedded: ass.systemText.includes(
        IMMERSIVE_LONGFORM_PROSE_BLOCK
      ),
    };
  });
  ...
  const soleOk = checks.every(
    (c) =>
      ...
      c.wording_d3 &&
      c.length_owner === 1 &&
      c.l1_combined_terminal === 0 &&
      c.scene_pacing === 1 &&
      c.scene_flow === 0 &&
      c.positive_prose_owner === 1 &&
      c.legacy_prose_owner === 0 &&
      c.positive_block_embedded
  );

  const preaudit = {
    phase: "G11-P1-PREAUDIT",
    seal_prev: [
      "DYNAMIC_DIALOGUE_BUDGET_PASS (#295)",
      "SERVER_CONTROL_PASS_LENGTH_STABILITY_REMAINS (#296)",
      "SINGLE_TERMINAL_LONGFORM_CONTRACT_FAIL (#297) — Arm W forbidden",
    ],
    sole_variable: "GENERIC_PROSE_OWNER → POSITIVE_PROSE_OWNER",
    ...
    frozen: {
      ...
      terminal_tail: "I1_D3_BYTE_IDENTICAL",
      scene_pacing_decision: "BYTE_IDENTICAL",
      arm: "V",
      arm_W_forbidden: true,
    },
    markers: {
      length_owner: USER_TAIL_LENGTH_OWNER_SENTENCE.slice(0, 40) + "…",
      dialogue_terminal: "[이번 응답 대화]",
      positive_prose: IMMERSIVE_LONGFORM_PROSE_MARKER,
      legacy_prose: LEGACY_IMMERSIVE_PROSE_MARKER,
    },
  ...
  save(DOCS, "00_API0.md", [
    ...
    "",
    `- prompt chars: legacy ${promptSize.legacy_immersive_chars} → positive ${promptSize.positive_longform_chars} (Δ ${promptSize.delta_chars})`,
    ...checks.map(
      (c) =>
        `- ${c.id}: budget=${c.resolved} len=${c.length_owner} dial=${c.terminal_owners} pacing=${c.scene_pacing}/${c.scene_flow} +prose=${c.positive_prose_owner} legacyProse=${c.legacy_prose_owner} L1=${c.l1_combined_terminal}`
    ),
    `- LIVE_CALL_READY: ${soleOk ? "YES" : "NO"}`,
    "",
  ].join("\n"));
  save(DOCS, "01_G11P1_PREAUDIT.json", preaudit);
  save(
    DOCS,
    "01_G11P1_PREAUDIT.md",
    [
      "# G11-P1 Preaudit",
      "",
      ...
      "```json",
      JSON.stringify(
        { prompt_size: promptSize, checks: preaudit.API0.checks },
        null,
        2
      ),
      "```",
  ...
  console.log(
    JSON.stringify(
      { LIVE_CALL_READY: soleOk, prompt_size: promptSize, checks },
      null,
      2
    )
  );
...
async function main() {
  ...
  console.log(
    `G11P1 phase=${PHASE} stage=${STAGE} model=${modelId} fixtures=${FIXTURE_FILTER.join(",")} draws=${DRAWS}`
  );
