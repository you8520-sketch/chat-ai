/**
 * Phase G10-SD2 — Scene pacing single-owner (REPLACE SCENE FLOW).
 *
 * PRODUCTION WIRE = 0 · MERGE = 0
 * Sole variable vs G10-SD1 P: REPLACE SCENE_FLOW_BLOCK with [SCENE PACING]
 * (no competing SCENE FLOW). Controller logic / cue wording BYTE_IDENTICAL.
 *
 *   PHASE=preaudit|live FIXTURES=N1S DRAWS=3 node --conditions=react-server --import tsx \
 *     scripts/rp-quality-g10sd2-single-owner.ts
 */
...
const OUT_ROOT =
  ...
  "/opt/cursor/artifacts/rp-quality-g10sd2-single-owner";
const DOCS =
  process.env.DOCS_DIR ?? "docs/audits/rp-scene-pacing-g10sd2";
const C10 =
...
async function runPreaudit(modelId: string) {
  ...
  const soleOk =
    A.canonDataSha === Q.canonDataSha &&
    P.canonDataSha === Q.canonDataSha &&
    A.historySha === Q.historySha &&
    A.userTailSha === Q.userTailSha &&
    JSON.stringify(A.generationConfig) === JSON.stringify(Q.generationConfig) &&
    Q.owners.scene_pacing === 1 &&
    Q.owners.scene_flow === 0 &&
    Q.owners.genre_scene_mode === 0 &&
    Q.owners.pacing_sot_count === 1 &&
    P.owners.pacing_sot_count === 2 &&
    Q.systemChars <= P.systemChars &&
    Q.applied.replacedSceneFlow &&
    Q.decision.pacingMode === "DYAD" &&
    Q.decision.motionLevel === "HOLD" &&
    JSON.stringify(P.decision) === JSON.stringify(Q.decision) &&
    Q.lastUserContent.includes(USER_TAIL_LENGTH_OWNER_SENTENCE);

  const preaudit = {
    phase: "G10-SD2-PREAUDIT",
    latest_main: "7f0c54b60e7ace11bc6e4eea9c820caadde24853",
    api_calls: 0,
    sole_variable: "REMOVE_COMPETING_SCENE_FLOW_OWNER",
    production_wire: "NOT_RUN",
    reference: "G10-SD1 stored P outputs (no redraw)",
    CURRENT_A: {
      system_tokens_est: A.systemTokensApprox,
      owners: countPacingOwners(A.systemText),
    },
    REFERENCE_P: {
    ...
    CANDIDATE_Q: {
      ...
      owners: Q.owners,
      decision: Q.decision,
      replacedSceneFlow: Q.applied.replacedSceneFlow,
      token_delta_vs_P: Q.systemTokensApprox - P.systemTokensApprox,
      char_delta_vs_P: Q.systemChars - P.systemChars,
    },
    invariants: {
      canon_equal: A.canonDataSha === Q.canonDataSha,
      history_equal: A.historySha === Q.historySha,
      user_tail_equal: A.userTailSha === Q.userTailSha,
      runtime_equal:
        JSON.stringify(A.generationConfig) ===
        JSON.stringify(Q.generationConfig),
      decision_p_q_identical: JSON.stringify(P.decision) === JSON.stringify(Q.decision),
      pacing_sot_P: P.owners.pacing_sot_count,
      pacing_sot_Q: Q.owners.pacing_sot_count,
      q_le_p_chars: Q.systemChars <= P.systemChars,
    },
    LIVE_CALL_READY: soleOk,
  };

  save(DOCS, "01_G10SD2_PREAUDIT.json", preaudit);
  save(
    DOCS,
    "01_G10SD2_PREAUDIT.md",
    [
      "# G10-SD2 Preaudit — Single Pacing Owner",
      "",
      `**LIVE_CALL_READY:** ${preaudit.LIVE_CALL_READY ? "YES" : "NO"}`,
      `**sole variable:** REMOVE_COMPETING_SCENE_FLOW_OWNER`,
      "",
      `| | P (SD1 insert) | Q (SD2 replace) |`,
      `|---|---:|---:|`,
      `| SCENE PACING | ${P.owners.scene_pacing} | ${Q.owners.scene_pacing} |`,
      `| SCENE FLOW | ${P.owners.scene_flow} | ${Q.owners.scene_flow} |`,
      `| pacing SoT | ${P.owners.pacing_sot_count} | ${Q.owners.pacing_sot_count} |`,
      `| system chars | ${P.systemChars} | ${Q.systemChars} (Δ ${Q.systemChars - P.systemChars}) |`,
      `| decision | ${P.decision.pacingMode}/${P.decision.motionLevel} | ${Q.decision.pacingMode}/${Q.decision.motionLevel} |`,
      "",
    ].join("\n")
  );
  console.log(
    JSON.stringify(
      {
        ...
        ownersP: P.owners,
        ownersQ: Q.owners,
        deltaChars: Q.systemChars - P.systemChars,
      },
      null,
      2
    )
...
async function runLive(modelId: string) {
  ...
  for (const fixture of fixtures) {
    const assembled = await assembleArm({ modelId, arm: "Q", fixture });
    for (let draw = 1; draw <= DRAWS; draw++) {
      const cellId = `Gemini_${fixture.id}_Q_D${draw}`;
      console.log(
        `\n=== ${cellId} mode=${assembled.decision.pacingMode}/${assembled.decision.motionLevel} sot=${assembled.owners.pacing_sot_count} ===`
      );
...
async function main() {
  ...
  console.log(
    `G10SD2 phase=${PHASE} model=${modelId} fixtures=${FIXTURE_FILTER.join(",")} draws=${DRAWS}`
  );
