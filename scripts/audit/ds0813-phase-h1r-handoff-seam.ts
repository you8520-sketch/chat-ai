/**
 * Phase H1R: assemble the frozen PR #560 Gemini → DeepSeek handoff fixture
...
const EVIDENCE = path.join(ROOT, "data/ds0813-phase-h1r-handoff-seam");
...
function flagsFor(text: string) {
  ...
  const deliberate = firstMatchSentence(
    text,
    /렌(?:이|은|가)?\s*(?:태형|라이크)?(?:의)?\s*(?:몸을\s*벽|벽\s*쪽|벽을\s*향해|밀어붙|손을\s*(?:뻗|옮|가져|넣)|끌어당|잡아끌|문을\s*열고|자리를\s*옮|물건을\s*(?:집|챙|사용))/
  );
  ...
  const newLoc = firstMatchSentence(
    text,
    /응접실|회의실|옥상|지하\s*주차장|대기실|감시실|통제실|의무실/
  );
  const sceneFact = firstMatchSentence(
    text,
    /(?:사이렌|비상벨|경보가\s*울|순찰대가|새로운\s*능력|각성했)/
  );
  const forced = firstMatchSentence(
    text,
    /계속할\s*거면|원하면\s*말해|허락해\s*줄래|어떻게\s*할래|선택해|A\s*아니면\s*B/
  );
  const questionThen = firstMatchSentence(
    text,
    /\?[^\n]{0,40}[\s\S]{8,80}(?:그대로|이어서|다시)/
  );
  ...
  return {
    CHARACTER_VOICE_SEAM: flagWithEvidence(
      genericLine ? "UNCERTAIN" : false,
      genericLine
    ),
    PROSE_RHYTHM_SEAM: flagWithEvidence(false, null),
    DIALOGUE_STYLE_SEAM: flagWithEvidence(
      genericLine ? "UNCERTAIN" : false,
      genericLine
    ),
    ...
    NEW_FORCED_CHOICE_AXIS: flagWithEvidence(Boolean(forced), forced),
    ...
    QUESTION_THEN_CONTINUES_WITHOUT_USER_REPLY: flagWithEvidence(
      Boolean(questionThen),
      questionThen
    ),
    UNDER_LENGTH: flagWithEvidence(text.length < FLOOR, text.length < FLOOR ? `VISIBLE_CHARS=${text.length}` : null),
...
function proveAcceptance(input: {
  ...
  const acceptance = {
    ...
    VISIBLE_GEMINI_SCENE_CONTINUITY_PRESERVED: visible && !canonRepair,
  ...
  if (acceptance.CURRENT_USER_NEWEST_STATE_PRESERVED !== true) {
    failures.push("CURRENT_USER_NEWEST_STATE_PRESERVED=false");
  }
  if (acceptance.VISIBLE_GEMINI_SCENE_CONTINUITY_PRESERVED !== true) {
    failures.push("VISIBLE_GEMINI_SCENE_CONTINUITY_PRESERVED=false");
  }
  ...
  if (failures.length) {
    throw new Error(`H1R assembly acceptance FAILED (no provider calls): ${failures.join("; ")}`);
  }
  return acceptance;
}
