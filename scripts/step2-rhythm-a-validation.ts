/**
 * Step 2 incremental validation — RHYTHM A only ([RHYTHM] line 3).
 *
 * 4 scenes × 5 runs × Before/After = 20 pairs (40 API calls)
 *
 * Usage:
 *   npx tsx scripts/step2-rhythm-a-validation.ts --fresh
 *   npx tsx scripts/step2-rhythm-a-validation.ts --analyze-only
 *   npx tsx scripts/step2-rhythm-a-validation.ts --dry-run
 */
...
import {
  buildProseStyleSectionForStep2Rhythm,
  PROSE_RHYTHM_LINE2_A,
  PROSE_RHYTHM_LINE2_BASELINE,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      const res = await callOpenRouterCompletion({
        ...
        maxTokens: 4096,
        requestKind: "step2-rhythm-a-validation",
      });
...
async function applyRhythmAIfKeep(keep: boolean): Promise<void> {
  if (!keep) return;
  const { readFile, writeFile } = await import("node:fs/promises");
  const path = join(process.cwd(), "src", "lib", "advancedProseNsfwGuidelines.ts");
  let src = await readFile(path, "utf8");
  if (!src.includes(PROSE_RHYTHM_LINE2_BASELINE)) {
    console.log("RHYTHM A already applied or baseline line missing — skip file apply");
    return;
  }
  src = src.replace(PROSE_RHYTHM_LINE2_BASELINE, PROSE_RHYTHM_LINE2_A);
  await writeFile(path, src, "utf8");
  console.log(`Applied RHYTHM A to ${path}`);
}
...
async function main() {
  ...
  const checkpointPath = join(outDir, "step2-rhythm-a-checkpoint.json");
  const resultPath = join(outDir, "step2-rhythm-a-validation.json");
  ...
  const bundleBefore = buildAdvancedProseNsfwGuidelines({
    nsfwEnabled: true,
    proseStyleSection: buildProseStyleSectionForStep2Rhythm("baseline"),
  });
  const bundleAfter = buildAdvancedProseNsfwGuidelines({
    nsfwEnabled: true,
    proseStyleSection: buildProseStyleSectionForStep2Rhythm("rhythm-a"),
  });
  writeFileSync(join(outDir, "prose-bundle-step2-rhythm-baseline.txt"), bundleBefore, "utf8");
  writeFileSync(join(outDir, "prose-bundle-step2-rhythm-a.txt"), bundleAfter, "utf8");

  const totalPairs = PROSE_VARIATION_SCENES.length * runsPerScene;
  const totalApiCalls = totalPairs * 2;
  console.log("=== Step 2 RHYTHM A Validation ===");
  console.log(`Scenes: ${PROSE_VARIATION_SCENES.length} | Runs: ${runsPerScene} | Paired jobs: ${totalPairs} | API calls: ${totalApiCalls}`);
  console.log("\nBefore [RHYTHM] line3:");
  console.log(`  ${PROSE_RHYTHM_LINE2_BASELINE}`);
  console.log("\nAfter [RHYTHM] line3 (A):");
  console.log(`  ${PROSE_RHYTHM_LINE2_A}`);
  ...
  const result = {
    test: "step2-rhythm-a-incremental",
  ...
  console.log(`\n=== Verdict: ${verdict.keep ? "KEEP RHYTHM A" : "ROLLBACK"} ===`);
  console.log(verdict.reason);
  console.log(`Wrote ${resultPath}`);

  if (!analyzeOnly) {
    if (verdict.keep) {
      await applyRhythmAIfKeep(true);
    } else {
