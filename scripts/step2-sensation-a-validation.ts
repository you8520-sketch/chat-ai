/**
 * Step 2 incremental validation — SENSATION A only ([SENSATION] line 1).
 *
 * 4 scenes × 5 runs × Before/After = 20 pairs (40 API calls)
 *
 * Usage:
 *   npx tsx scripts/step2-sensation-a-validation.ts --fresh
 *   npx tsx scripts/step2-sensation-a-validation.ts --analyze-only
 *   npx tsx scripts/step2-sensation-a-validation.ts --dry-run
 */
...
import {
  buildProseStyleSectionForStep2Sensation,
  PROSE_SENSATION_LINE0_A,
  PROSE_SENSATION_LINE0_BASELINE,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      const res = await callOpenRouterCompletion({
        ...
        maxTokens: 4096,
        requestKind: "step2-sensation-a-validation",
      });
...
async function applySensationAIfKeep(keep: boolean): Promise<void> {
  if (!keep) return;
  const { readFile, writeFile } = await import("node:fs/promises");
  const path = join(process.cwd(), "src", "lib", "advancedProseNsfwGuidelines.ts");
  let src = await readFile(path, "utf8");
  if (!src.includes(PROSE_SENSATION_LINE0_BASELINE)) {
    console.log("SENSATION A already applied or baseline line missing — skip file apply");
    return;
  }
  src = src.replace(PROSE_SENSATION_LINE0_BASELINE, PROSE_SENSATION_LINE0_A);
  await writeFile(path, src, "utf8");
  console.log(`Applied SENSATION A to ${path}`);
}
...
async function main() {
  ...
  const checkpointPath = join(outDir, "step2-sensation-a-checkpoint.json");
  const resultPath = join(outDir, "step2-sensation-a-validation.json");
  ...
  const bundleBefore = buildAdvancedProseNsfwGuidelines({
    nsfwEnabled: true,
    proseStyleSection: buildProseStyleSectionForStep2Sensation("baseline"),
  });
  const bundleAfter = buildAdvancedProseNsfwGuidelines({
    nsfwEnabled: true,
    proseStyleSection: buildProseStyleSectionForStep2Sensation("sensation-a"),
  });
  writeFileSync(join(outDir, "prose-bundle-step2-sensation-baseline.txt"), bundleBefore, "utf8");
  writeFileSync(join(outDir, "prose-bundle-step2-sensation-a.txt"), bundleAfter, "utf8");

  const totalPairs = PROSE_VARIATION_SCENES.length * runsPerScene;
  const totalApiCalls = totalPairs * 2;
  console.log("=== Step 2 SENSATION A Validation ===");
  console.log(`Scenes: ${PROSE_VARIATION_SCENES.length} | Runs: ${runsPerScene} | Paired jobs: ${totalPairs} | API calls: ${totalApiCalls}`);
  console.log("\nBefore [SENSATION] line1:");
  console.log(`  ${PROSE_SENSATION_LINE0_BASELINE}`);
  console.log("\nAfter [SENSATION] line1 (A):");
  console.log(`  ${PROSE_SENSATION_LINE0_A}`);
  ...
  const result = {
    test: "step2-sensation-a-incremental",
  ...
  console.log(`\n=== Verdict: ${verdict.keep ? "KEEP SENSATION A" : "ROLLBACK"} ===`);
  console.log(verdict.reason);
  console.log(`Wrote ${resultPath}`);

  if (!analyzeOnly) {
    if (verdict.keep) {
      await applySensationAIfKeep(true);
    } else {
