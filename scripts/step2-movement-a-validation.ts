/**
 * Step 2 incremental validation — MOVEMENT A only ([MOVEMENT & SPACE] line 2).
 *
 * 4 scenes × 5 runs × Before/After = 20 pairs (40 API calls)
 *
 * Usage:
 *   npx tsx scripts/step2-movement-a-validation.ts --fresh
 *   npx tsx scripts/step2-movement-a-validation.ts --analyze-only
 *   npx tsx scripts/step2-movement-a-validation.ts --dry-run
 */
...
import {
  buildProseStyleSectionForStep2Movement,
  PROSE_MOVEMENT_LINE1_A,
  PROSE_MOVEMENT_LINE1_BASELINE,
  ...
  for (let attempt = 0; attempt < maxAttempts; attempt++) {
    try {
      const res = await callOpenRouterCompletion({
        ...
        maxTokens: 4096,
        requestKind: "step2-movement-a-validation",
      });
...
async function applyMovementAIfKeep(keep: boolean): Promise<void> {
  if (!keep) return;
  const { readFile, writeFile } = await import("node:fs/promises");
  const path = join(process.cwd(), "src", "lib", "advancedProseNsfwGuidelines.ts");
  let src = await readFile(path, "utf8");
  if (!src.includes(PROSE_MOVEMENT_LINE1_BASELINE)) {
    console.log("MOVEMENT A already applied or baseline line missing — skip file apply");
    return;
  }
  src = src.replace(PROSE_MOVEMENT_LINE1_BASELINE, PROSE_MOVEMENT_LINE1_A);
  await writeFile(path, src, "utf8");
  console.log(`Applied MOVEMENT A to ${path}`);
}
...
async function main() {
  ...
  const checkpointPath = join(outDir, "step2-movement-a-checkpoint.json");
  const resultPath = join(outDir, "step2-movement-a-validation.json");
  ...
  const bundleBefore = buildAdvancedProseNsfwGuidelines({
    nsfwEnabled: true,
    proseStyleSection: buildProseStyleSectionForStep2Movement("baseline"),
  });
  const bundleAfter = buildAdvancedProseNsfwGuidelines({
    nsfwEnabled: true,
    proseStyleSection: buildProseStyleSectionForStep2Movement("movement-a"),
  });
  writeFileSync(join(outDir, "prose-bundle-step2-movement-baseline.txt"), bundleBefore, "utf8");
  writeFileSync(join(outDir, "prose-bundle-step2-movement-a.txt"), bundleAfter, "utf8");

  const totalPairs = PROSE_VARIATION_SCENES.length * runsPerScene;
  const totalApiCalls = totalPairs * 2;
  console.log("=== Step 2 MOVEMENT A Validation ===");
  console.log(`Scenes: ${PROSE_VARIATION_SCENES.length} | Runs: ${runsPerScene} | Paired jobs: ${totalPairs} | API calls: ${totalApiCalls}`);
  console.log("\nBefore [MOVEMENT & SPACE] line2:");
  console.log(`  ${PROSE_MOVEMENT_LINE1_BASELINE}`);
  console.log("\nAfter [MOVEMENT & SPACE] line2 (A):");
  console.log(`  ${PROSE_MOVEMENT_LINE1_A}`);
  ...
  const result = {
    test: "step2-movement-a-incremental",
  ...
  console.log(`\n=== Verdict: ${verdict.keep ? "KEEP MOVEMENT A" : "ROLLBACK"} ===`);
  console.log(verdict.reason);
  console.log(`Wrote ${resultPath}`);

  if (!analyzeOnly) {
    if (verdict.keep) {
      await applyMovementAIfKeep(true);
    } else {
