/**
 * Phase G10-D3 — Dynamic server dialogue budget (terminal ceiling).
 *
 * PRODUCTION WIRE = 0 · MERGE = 0
 * Baseline: G10-SD2 Q + G10-D2 terminal location.
 * Sole architecture: Arm V resolves maxBlocks 4|5|6|null before append.
 *
 *   PHASE=preaudit|live FIXTURES=N1S,N2,C6 DRAWS=2 node --conditions=react-server --import tsx \
 *     scripts/rp-quality-g10d3-dynamic-dialogue-budget.ts
 */
...
const OUT_ROOT =
  ...
  "/opt/cursor/artifacts/rp-quality-g10d3-dynamic-dialogue-budget";
const DOCS =
  process.env.DOCS_DIR ?? "docs/audits/rp-scene-pacing-g10d3";
const C10 =
...
const FIXTURE_FILTER = (process.env.FIXTURES ?? "N1S,N2,C6")
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);

const FIXTURE_ALIAS: Record<string, string> = {
  N1S: "Q4",
  N2: "E5",
  C6: "C6",
};
  ...
  const decision: ScenePacingDecision = resolveScenePacingDecision({
    ...
    establishedActiveCastNames: opts.party
      ...
      : contentKind === "simulation"
        ? ["병사A", "병사B", "정찰대"]
        : undefined,
    chatId: `g10d3-${opts.fixture.id}-${opts.arm}`,
  });
...
async function runPreaudit(modelId: string) {
  const all = JSON.parse(readFileSync(FIX_PATH, "utf8")) as {
    fixtures: NFixture[];
  };
  const n1s = all.fixtures.find((f) => f.id === "N1S")!;
  const n2 = all.fixtures.find((f) => f.id === "N2")!;
  const c6 = all.fixtures.find((f) => f.id === "C6")!;

  const Q4 = await assembleArm({ modelId, arm: "V", fixture: n1s });
  const E5 = await assembleArm({ modelId, arm: "V", fixture: n2 });
  const C6 = await assembleArm({ modelId, arm: "V", fixture: c6 });
  const party = await assembleArm({
    modelId,
    arm: "V",
    fixture: n1s,
    party: true,
  });
  const sim = await assembleArm({
    modelId,
    arm: "V",
    fixture: n1s,
    contentKind: "simulation",
  ...
  // False-positive assemble via synthetic fixture
  const fpFixture: NFixture = {
    id: "FP",
    title: "false positive radio mention",
    role: "API0",
    userInput:
      "*렌이 컵을 내려놓는다.* 어제 무전기가 고장났어. 오늘은 좀 조용하네.",
    historyAfterGreeting: n1s.historyAfterGreeting,
  };
  const FP = await assembleArm({ modelId, arm: "V", fixture: fpFixture });

  const soleOk =
    Q4.dialogueBudget?.maxBlocks === 4 &&
    E5.dialogueBudget?.maxBlocks === 5 &&
    C6.dialogueBudget?.maxBlocks === 6 &&
    party.dialogueBudget?.maxBlocks == null &&
    sim.dialogueBudget?.maxBlocks == null &&
    FP.dialogueBudget?.maxBlocks === 4 &&
    Q4.systemDialogueOwners.dialogue_block_owner === 0 &&
    Q4.terminalOwners.terminal_dialogue_budget_owner === 1 &&
    party.terminalOwners.terminal_dialogue_budget_owner === 0 &&
    sim.terminalOwners.terminal_dialogue_budget_owner === 0 &&
    C6.owners.scene_pacing === 1 &&
    C6.owners.scene_flow === 0 &&
    !/보통 1~3개면 충분/.test(Q4.lastUserContent) &&
    Q4.lastUserContent.includes(USER_TAIL_LENGTH_OWNER_SENTENCE) &&
    /최대 4개 블록/.test(Q4.lastUserContent) &&
    /최대 5개 블록/.test(E5.lastUserContent) &&
    /최대 6개 블록/.test(C6.lastUserContent);

  const preaudit = {
    phase: "G10-D3-PREAUDIT",
    latest_main: "7f0c54b60e7ace11bc6e4eea9c820caadde24853",
    api_calls: 0,
    sole_variable: "SERVER_RESOLVED_DIALOGUE_CEILING",
    production_wire: "NOT_RUN",
    API0: {
      Q4: {
        mode: `${Q4.decision.pacingMode}/${Q4.decision.motionLevel}`,
        budget: Q4.dialogueBudget,
        terminal: Q4.terminalOwners.terminal_dialogue_budget_owner,
      },
      E5: {
        mode: `${E5.decision.pacingMode}/${E5.decision.motionLevel}`,
        budget: E5.dialogueBudget,
        terminal: E5.terminalOwners.terminal_dialogue_budget_owner,
      },
      C6: {
        mode: `${C6.decision.pacingMode}/${C6.decision.motionLevel}`,
        budget: C6.dialogueBudget,
        terminal: C6.terminalOwners.terminal_dialogue_budget_owner,
      },
      party: {
        budget: party.dialogueBudget,
        terminal: party.terminalOwners.terminal_dialogue_budget_owner,
      },
      simulation: {
        budget: sim.dialogueBudget,
        terminal: sim.terminalOwners.terminal_dialogue_budget_owner,
      },
      false_positive: {
        mode: `${FP.decision.pacingMode}/${FP.decision.motionLevel}`,
        budget: FP.dialogueBudget,
        terminal: FP.terminalOwners.terminal_dialogue_budget_owner,
      },
    },
  ...
  save(DOCS, "00_API0.md", [
    ...
    "",
    `- Q4: ${Q4.decision.pacingMode}/${Q4.decision.motionLevel} → max=${Q4.dialogueBudget?.maxBlocks}`,
    `- E5: ${E5.decision.pacingMode}/${E5.decision.motionLevel} → max=${E5.dialogueBudget?.maxBlocks}`,
    `- C6: ${C6.decision.pacingMode}/${C6.decision.motionLevel} → max=${C6.dialogueBudget?.maxBlocks} (${C6.dialogueBudget?.reason})`,
    `- party/sim terminal: ${party.terminalOwners.terminal_dialogue_budget_owner}/${sim.terminalOwners.terminal_dialogue_budget_owner}`,
    `- false-positive: max=${FP.dialogueBudget?.maxBlocks}`,
    `- LIVE_CALL_READY: ${soleOk ? "YES" : "NO"}`,
    "",
  ].join("\n"));
  save(DOCS, "01_G10D3_PREAUDIT.json", preaudit);
  save(
    DOCS,
    "01_G10D3_PREAUDIT.md",
    [
      "# G10-D3 Preaudit",
      "",
...
async function runLive(modelId: string) {
  ...
  for (const fixture of fixtures) {
    const alias = FIXTURE_ALIAS[fixture.id] ?? fixture.id;
    const assembled = await assembleArm({ modelId, arm: "V", fixture });
    for (let draw = 1; draw <= DRAWS; draw++) {
      const cellId = `Gemini_${alias}_V_D${draw}`;
      console.log(
        `\n=== ${cellId} mode=${assembled.decision.pacingMode}/${assembled.decision.motionLevel} budget=${assembled.dialogueBudget?.maxBlocks} reason=${assembled.dialogueBudget?.reason} ===`
      );
  ...
  console.log(JSON.stringify({ apiCalls, cells: cells.map((c) => ({
    id: c.cell_id,
...
async function main() {
  ...
  console.log(
    `G10D3 phase=${PHASE} model=${modelId} fixtures=${FIXTURE_FILTER.join(",")} draws=${DRAWS}`
  );
