/**
 * Phase G11-L1 — Single Terminal Longform Response Contract.
 *
 * FREEZE (BYTE_IDENTICAL): Scene Pacing decision, communication demand,
 * dialogue budget resolver (4/5/6/null). Sole variable: REPLACE standalone
 * USER_TAIL_LENGTH_OWNER_SENTENCE + [이번 응답 대화] with one [이번 응답]
 * LONGFORM RESPONSE CONTRACT (Arm W).
 *
 * PRODUCTION WIRE = 0 · MERGE = 0 · retry/continuation/repair = 0
 *
 *   PHASE=preaudit|live STAGE=1|2 FIXTURES=B,D,F DRAWS=2 \
 *     node --conditions=react-server --import tsx \
 *     scripts/rp-quality-g11l1-single-terminal-longform-contract.ts
 */
...
const OUT_ROOT =
  ...
  "/opt/cursor/artifacts/rp-quality-g11l1-longform-contract";
const DOCS =
  process.env.DOCS_DIR ?? "docs/audits/rp-longform-contract-g11l1";
const FIX_PATH =
  "docs/audits/rp-integrated-server-control-g11i1/fixtures/G11_I1_FIXTURES.json";
const DRAWS = Number(process.env.DRAWS ?? "2");
const STAGE = (process.env.STAGE ?? "1").trim();
const PHASE =
  (process.env.PHASE as "preaudit" | "live" | undefined) ??
  (process.argv.includes("--live") ? "live" : "preaudit");
const DEFAULT_FIXTURES =
  STAGE === "2" ? "A,C,E" : STAGE === "all" ? "A,B,C,D,E,F" : "B,D,F";
const FIXTURE_FILTER = (process.env.FIXTURES ?? DEFAULT_FIXTURES)
  .split(",")
  .map((s) => s.trim())
  .filter(Boolean);

const SHORT_THRESHOLD = 2000;
...
async function assembleFixture(opts: {
  ...
  return {
    ...
    longformOwners: countTerminalLongformResponseOwners(
...
async function runPreaudit(modelId: string) {
  // API=0 always validates the full A–F matrix regardless of live stage filter.
  const allRaw = JSON.parse(readFileSync(FIX_PATH, "utf8")) as {
    fixtures: G11Fixture[];
  };
  const allFixtures = allRaw.fixtures;
  const byId: Record<string, Awaited<ReturnType<typeof assembleFixture>>> = {};
  for (const f of allFixtures) {
    byId[f.id] = await assembleFixture({ modelId, fixture: f });
  }

  const checks = allFixtures.map((f) => {
    const ass = byId[f.id]!;
    const max = ass.dialogueBudget?.maxBlocks ?? null;
    const lo = ass.longformOwners;
    const contract = renderTerminalLongformResponseContract(max);
    const contractOk = ass.lastUserContent.includes(contract);
    const maxMentionCount =
      max == null
        ? 0
        : (
            ass.lastUserContent.match(
              new RegExp(`최대 ${max}개 블록`, "g")
            ) ?? []
          ).length;
    const genreSceneMode = (ass.systemText.match(/\[GENRE SCENE MODE\]/g) ?? [])
      .length;
    return {
      id: f.id,
      domain: f.domain,
      ...
      longform_terminal: lo.terminal_longform_response_owner,
      old_length_owner: lo.standalone_length_owner,
      old_dialogue_terminal: lo.standalone_dialogue_terminal,
      budget_match: max === f.expectedBudget,
      contract_match: contractOk,
      max_number_mentions: maxMentionCount,
      scene_pacing: ass.owners.scene_pacing,
      scene_flow: ass.owners.scene_flow,
      genre_scene_mode: genreSceneMode,
      length_owner_sentence_absent: !ass.lastUserContent.includes(
        USER_TAIL_LENGTH_OWNER_SENTENCE
      ),
      ends_with_contract: ass.lastUserContent.trimEnd().endsWith(
        contract.trimEnd()
      ),
    };
  });

  const soleOk = checks.every(
    (c) =>
      c.budget_match &&
      c.longform_terminal === 1 &&
      c.old_length_owner === 0 &&
      c.old_dialogue_terminal === 0 &&
      c.contract_match &&
      c.length_owner_sentence_absent &&
      c.ends_with_contract &&
      c.scene_pacing === 1 &&
      c.scene_flow === 0 &&
      c.genre_scene_mode === 0 &&
      (c.expected == null
        ? c.max_number_mentions === 0
        : c.max_number_mentions === 1)
  );

  const preaudit = {
    phase: "G11-L1-PREAUDIT",
    seal_prev: "SERVER_CONTROL_PASS_LENGTH_STABILITY_REMAINS (#296)",
    sole_variable: "SINGLE_TERMINAL_LONGFORM_RESPONSE_CONTRACT",
    latest_main: "7f0c54b60e7ace11bc6e4eea9c820caadde24853",
    api_calls: 0,
    frozen: {
      budgets: { quiet: 4, exploration: 5, operation: 6, ensemble: null },
      scene_pacing_decision: "BYTE_IDENTICAL",
      communication_demand: "BYTE_IDENTICAL",
      dialogue_budget_resolver: "BYTE_IDENTICAL",
      no_new_negatives: true,
    },
    architecture: {
      old_length_terminal: 0,
      old_dialogue_terminal: 0,
      new_combined_longform_terminal: 1,
      length_semantics: "base>=3200 no upper ceiling",
    },
    fixtures_live_default: FIXTURE_FILTER,
    API0: { checks },
  ...
  save(DOCS, "00_API0.md", [
    ...
    ...checks.map(
      (c) =>
        `- ${c.id}: budget=${c.resolved} longform=${c.longform_terminal} oldLen=${c.old_length_owner} oldDial=${c.old_dialogue_terminal} pacing=${c.scene_pacing}/${c.scene_flow} genre=${c.genre_scene_mode}`
    ),
    `- LIVE_CALL_READY: ${soleOk ? "YES" : "NO"}`,
    "",
  ].join("\n"));
  save(DOCS, "01_G11L1_PREAUDIT.json", preaudit);
  save(
    DOCS,
    "01_G11L1_PREAUDIT.md",
    [
      "# G11-L1 Preaudit",
      "",
  ...
  console.log(
    JSON.stringify({ LIVE_CALL_READY: soleOk, checks }, null, 2)
  );
  return preaudit;
}

function buildCapLengthMatrix(cells: Array<Record<string, unknown>>) {
  const matrix = {
    SHORT_CAP_REACHED: 0,
    SHORT_CAP_NOT_REACHED: 0,
    LONG_CAP_REACHED: 0,
    LONG_CAP_NOT_REACHED: 0,
    UNCAPPED: 0,
  };
  for (const c of cells) {
    const chars = Number(c.visible_chars);
    const max = (c.budget as { maxBlocks: number | null } | null)?.maxBlocks;
    const blocks = Number(c.speech_blocks);
    if (max == null) {
      matrix.UNCAPPED += 1;
      continue;
    }
    const short = chars < SHORT_THRESHOLD;
    const long = chars >= LONG_THRESHOLD;
    const reached = blocks === max;
    if (short && reached) matrix.SHORT_CAP_REACHED += 1;
    else if (short && !reached) matrix.SHORT_CAP_NOT_REACHED += 1;
    else if (long && reached) matrix.LONG_CAP_REACHED += 1;
    else if (long && !reached) matrix.LONG_CAP_NOT_REACHED += 1;
    else if (reached) matrix.SHORT_CAP_NOT_REACHED += 0; // mid band ignored in named buckets
    // mid (2000-2999) tracked only via length_summary; matrix focuses short/long extremes
  }
  // Also count mid-band into NOT_REACHED/REACHED via extended fields
  const mid = {
    MID_CAP_REACHED: 0,
    MID_CAP_NOT_REACHED: 0,
  };
  for (const c of cells) {
    const chars = Number(c.visible_chars);
    const max = (c.budget as { maxBlocks: number | null } | null)?.maxBlocks;
    ...
    if (chars >= SHORT_THRESHOLD && chars < LONG_THRESHOLD) {
      if (blocks === max) mid.MID_CAP_REACHED += 1;
      else mid.MID_CAP_NOT_REACHED += 1;
    }
  }
  return { ...matrix, ...mid };
}
...
async function runLive(modelId: string) {
  ...
  for (const fixture of fixtures) {
    ...
    for (let draw = 1; draw <= DRAWS; draw++) {
      const cellId = `Gemini_${fixture.id}_W_D${draw}`;
      console.log(
      ...
      const maxBlocks = assembled.dialogueBudget?.maxBlocks ?? null;
      const dialogueCapReached =
        maxBlocks != null && speech.speech_blocks === maxBlocks;
      ...
      cells.push({
        ...
        DIALOGUE_CAP_REACHED: dialogueCapReached,
      ...
      save(
        ...
        [
          ...
          `- intimate: ${assembled.decision.intimateDyad}`,
          `- budget: max=${maxBlocks} reason=${assembled.dialogueBudget?.reason} demand=${assembled.dialogueBudget?.communicationDemand}`,
          `- expected_budget: ${fixture.expectedBudget}`,
          `- visible_chars: ${chars}`,
          `- speech_blocks: ${speech.speech_blocks}`,
          `- DIALOGUE_CAP_REACHED: ${dialogueCapReached}`,
          ...
          "",
          "```text",
          assembled.lastUserContent.slice(-450),
          "```",
          "",
          "## user_input",
          "",
      ...
      console.log(
        `${cellId}: chars=${chars} blocks=${speech.speech_blocks}/${maxBlocks ?? "∞"} cap_reached=${dialogueCapReached}`
      );
    }
  }
  ...
  const capMatrix = buildCapLengthMatrix(cells);

  const live = {
    phase: `G11-L1-STAGE${STAGE}-LIVE`,
    api_calls: apiCalls,
    target_calls: fixtures.length * DRAWS,
    fixtures: fixtures.map((f) => f.id),
    sole_variable: "SINGLE_TERMINAL_LONGFORM_RESPONSE_CONTRACT",
    i1_bdf_baseline_mean_approx: 2148,
    length_summary: {
      ...
      max: charList.length ? Math.max(...charList) : 0,
    },
    cap_length_matrix: capMatrix,
    cells,
    human_review: "PENDING",
    production_wire: "NOT_RUN",
    merge: "NOT_RUN",
  };
  const stageDir = STAGE === "2" ? "stage2" : "stage1";
  save(join(DOCS, stageDir), "01_STAGE_LIVE.json", live);
  console.log(
    JSON.stringify(
      {
        apiCalls,
        ...
        cells: cells.map((c) => ({
          ...
          cap_reached: c.DIALOGUE_CAP_REACHED,
...
async function main() {
  ...
  mkdirSync(join(DOCS, "stage2"), { recursive: true });
  ...
  console.log(
    `G11L1 phase=${PHASE} stage=${STAGE} model=${modelId} fixtures=${FIXTURE_FILTER.join(",")} draws=${DRAWS}`
  );
