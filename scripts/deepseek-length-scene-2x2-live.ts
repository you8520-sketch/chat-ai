/**
 * Live ARM C / D for Audit 42 (Length × Scene 2×2).
 * Frozen A/B outputs are NOT regenerated.
 *
 * Env:
 *   PROD_BASE, PROD_COOKIE_FILE, OUT_ROOT
 *   ARM=C|D  (caller toggles Railway canary variant)
 */
import {
  mkdirSync,
  writeFileSync,
...
const ARM = (process.env.ARM ?? "C").toUpperCase() as "C" | "D";
if (ARM !== "C" && ARM !== "D") {
  throw new Error(`ARM must be C or D, got ${ARM}`);
}
const OUT_ROOT =
  process.env.OUT_ROOT ??
  `/opt/cursor/artifacts/deepseek-length-scene-2x2/arm-${ARM}`;
const EXPECTED_VARIANT =
  ARM === "C" ? "ds_triple_owner_scene_off" : "ds_single_owner_scene_off";
const EXPECTED_LENGTH_OWNERS = ARM === "C" ? 3 : 1;
...
async function postChat(opts: {
  ...
  const body: Record<string, unknown> = {
    ...
    isAdultMode: false,
    isNsfwMode: false,
    clientRequestId: `ds_2x2_${ARM}_${opts.tag}_${Date.now().toString(36)}`,
  };
