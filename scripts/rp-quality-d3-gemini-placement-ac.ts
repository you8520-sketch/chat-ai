/**
 * Phase D3 — Gemini context-boundary placement A/C (Stage 1).
 *
 * A = production (continuity absent)
 * T = D2 terminal_system — HISTORICAL ONLY (no live re-call)
 * C = same wording at context-boundary (before [OUTPUT LAYOUT])
 *
 * Fixtures: G6-T1 / G5 / G3 × A/C = 6 successful calls
 *
 *   node --conditions=react-server --import tsx scripts/rp-quality-d3-gemini-placement-ac.ts
 */
...
const OUT_ROOT =
  process.env.OUT_ROOT ?? "/opt/cursor/artifacts/rp-quality-d3-placement";
const DOCS = process.env.DOCS_DIR ?? "docs/audits/rp-quality-v2-gemini";
const RAW_DOCS = join(DOCS, "d3", "raw");
...
const FIXTURES: FixtureSpec[] = [
  {
    ...
    provenance:
      "D3 G6-T1 — primary length-recovery signal vs D2 T=658",
    measures: [
  ...
  },
  {
    ...
    provenance: "D3 G5 — keep D2 T intro/recital gains if possible",
    measures: [
      "INTRO_REPLAY",
      "SETTING_RECITAL",
      "CURRENT_INPUT_REPLAY",
  ...
  },
  {
    ...
    provenance: "D3 G3 — canon non-inferior control",
    measures: [
      "ACTIVE_CANON_USE",
      "CHARACTER_FIDELITY",
      "SETTING_RECITAL",
...
function armToPlacement(arm: LiveArm): GeminiContinuityPlacement {
  return arm === "C" ? "context_boundary" : "absent";
}
...
async function assembleCell(opts: {
  ...
  const placement = armToPlacement(opts.arm);
  ...
  if (opts.arm === "C") {
    if (!applied.injected) {
      throw new Error("arm C must inject continuity at context boundary");
    }
    if (
      !contextBoundaryPreservesOtherSections({
        baselineSystem: built.systemPrompt,
        placedSystem: applied.systemPrompt,
        block: GEMINI_SCENE_CONTINUITY_BLOCK,
      })
    ) {
      throw new Error("arm C rewrote non-continuity sections — FAIL");
    }
    const contIdx = applied.systemPrompt.indexOf("[GEMINI SCENE CONTINUITY]");
    const layoutIdx = applied.systemPrompt.indexOf(OUTPUT_LAYOUT_BOUNDARY_MARKER);
    if (!(contIdx >= 0 && contIdx < layoutIdx)) {
      throw new Error("arm C continuity must sit before [OUTPUT LAYOUT]");
    }
  }
  if (opts.arm === "A" && applied.injected) {
    throw new Error("arm A must not inject continuity");
  }
  ...
  return {
    ...
    messagesSha: sha256(JSON.stringify((wire.requestBody as { messages?: unknown }).messages)),
    userTailSha: sha256(lastUser?.content ?? ""),
    greeting: String(ch.greeting ?? ""),
    injected: applied.injected,
    placement: applied.placement,
    insertMarker: applied.insertMarker,
...
function shufflePair(): [LiveArm, LiveArm] {
  const a: LiveArm[] = ["A", "C"];
...
async function main() {
  ...
  mkdirSync(join(DOCS, "d3"), { recursive: true });
  mkdirSync(RAW_DOCS, { recursive: true });

  if (!process.env.OPENROUTER_API_KEY?.trim()) {
    throw new Error("OPENROUTER_API_KEY required for D3 live A/C");
  }
  ...
  // Historical T reference (D2 B) — no live re-call
  const d2LivePath = join(DOCS, "d2", "01_STAGE1_LIVE.json");
  const d2Historical = existsSync(d2LivePath)
    ? JSON.parse(readFileSync(d2LivePath, "utf8"))
    : null;
  save(join(DOCS, "d3"), "01_T_HISTORICAL_REFERENCE.json", {
    note: "T = D2 terminal_system arm B — not re-called in D3",
    source: "docs/audits/rp-quality-v2-gemini/d2/",
    wording_sha256: sha256(GEMINI_SCENE_CONTINUITY_BLOCK),
    d2_stage1_excerpt: d2Historical
      ? (d2Historical.rows as Array<Record<string, unknown>>)
          .filter((r) => ["G5", "G6T1", "G3"].includes(String(r.fixture)))
          .map((r) => ({
            cell_id: r.cell_id,
            fixture: r.fixture,
            arm: r.arm,
  ...
  const hiddenPath = join(DOCS, "d3", "08_HIDDEN_MAP.json");
  let blindMap: Record<string, Record<"P" | "Q", LiveArm>>;
  if (existsSync(hiddenPath)) {
    ...
    save(join(DOCS, "d3"), "08_HIDDEN_MAP.json", {
      note: "Reveal only after human scoring seal",
      model: modelId,
      blindMap,
      arm_meanings: {
        A: "production — continuity absent",
        T: "D2 terminal_system — historical only",
        C: "context_boundary before [OUTPUT LAYOUT]",
      },
    });
  }

  save(join(DOCS, "d3"), "00_FIXTURE_PROVENANCE.json", {
    fixtures: FIXTURES,
    modelId,
    wording_sha256: sha256(GEMINI_SCENE_CONTINUITY_BLOCK),
    ...
    for (const arm of LIVE_ARMS) {
      ...
      if (existsSync(rawPath) && existsSync(join(dir, "meta.json"))) {
        ...
        fingerprints.push({
          ...
          insert_marker: assembled.insertMarker,
        });
        console.log(
          `\n=== ${cellId} arm=${arm} placement=${assembled.placement} injected=${assembled.injected} ===`
        );
      ...
      writeRawMd({
        ...
        placement: String(meta.placement ?? armToPlacement(arm)),
      });
      ...
      rows.push({
        ...
        fixture: spec.id,
        arm,
        placement: meta.placement ?? armToPlacement(arm),
        ...
        human: {
          ...
          OPENING_REPLAY_PARAGRAPHS: "PENDING",
          REPLAY_SAVED_CHARS_REPLACED_BY_NEW_SCENE: "PENDING",
          SCENE_FULLY_DEVELOPED: "PENDING",
          COMPLETION: "PENDING",
        },
      });
    }
  ...
  save(join(DOCS, "d3"), "00_FINGERPRINTS.json", { fingerprints });
  ...
  save(join(DOCS, "d3"), "02_STAGE1_LIVE.json", summary);
  save(
    join(DOCS, "d3"),
    "02_STAGE1_LIVE.md",
    [
