...
export default async function TabPage({
  ...
  switch (tab) {
    case "official":
      title = "공식 캐릭터";
      chars = db.prepare(`SELECT * FROM characters WHERE official=1 ${filter} ORDER BY likes DESC`).all(...fparams) as CharacterRow[];
      break;
    case "new":
      title = "실시간 신작";
      chars = db
    ...
    case "genre":
      ...
      if (g && g !== "전체" && CHARACTER_GENRES.includes(g as CharacterGenre)) {
        ...
        chars = db
          .prepare(`SELECT * FROM characters WHERE ${listableWhere()} ${filter} ORDER BY genre, likes DESC`)
          .all(...fparams) as CharacterRow[];
      }
      break;
    case "following":
      title = "팔로잉";
      if (!user) {
        return <Empty title="팔로잉" message="로그인 후 크리에이터를 팔로우하면 여기에 표시됩니다." cta="/login" ctaLabel="로그인" />;
      }
      chars = db
        .prepare(
    ...
    case "likes":
      title = "좋아요한 캐릭터";
      if (!user) {
        return <Empty title="좋아요" message="로그인 후 좋아요한 캐릭터가 여기에 표시됩니다." cta="/login" ctaLabel="로그인" />;
      }
      chars = db
        .prepare(
  ...
  return (
    <div className="mt-6">
      ...
      {tab === "ranking" && (
        <>
          <p className="mt-1 text-sm text-gray-500">{rankingPeriodDesc(rankingPeriod)}</p>
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          {GENRE_TABS.map((genre) => (
            <Link
              ...
              className={`rounded-full px-3 py-1 text-sm ${
                (g || "전체") === genre ? "bg-violet-600 text-white" : "bg-white/5 text-gray-300 hover:bg-white/10"
              }`}
            >
              {genre}
            </Link>
      ...
      {tab === "ranking" ? (
          ...
          {rankedChars.map((c, i) => {
            ...
            return (
              <li key={c.id}>
                <Link
                  href={href}
                  className="flex items-center gap-4 rounded-xl border border-white/5 bg-[#131626] p-3 hover:border-violet-500/40"
                >
                  <span
                    className={`w-8 shrink-0 text-center text-lg font-black ${i < 3 ? "text-violet-400" : "text-gray-500"}`}
                  >
                    {i + 1}
                  </span>
                  <div
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-semibold text-white">
                      {c.name}
                      {c.nsfw === 1 && (
                        <span className="ml-2 rounded bg-rose-600 px-1 text-[10px] font-bold">19</span>
                      )}
                    </p>
                  ...
                  <span className="shrink-0 text-sm text-gray-400">💬 {c.period_chats.toLocaleString()}</span>
                </Link>
              </li>
            );
          })}
        ...
        <p className="mt-10 text-center text-gray-500">표시할 캐릭터가 없습니다.</p>
      )}
    </div>
  );
}

function Empty({ title, message, cta, ctaLabel }: { title: string; message: string; cta: string; ctaLabel: string }) {
  return (
    <div className="mt-6">
      <h1 className="text-xl font-bold text-white">{title}</h1>
      <div className="mt-10 text-center">
        <p className="text-gray-400">{message}</p>
        <Link href={cta} className="mt-4 inline-block rounded-full bg-violet-600 px-6 py-2 font-semibold text-white">
          {ctaLabel}
        </Link>
      </div>
    </div>
