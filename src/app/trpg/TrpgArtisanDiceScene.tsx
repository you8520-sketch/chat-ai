...
const HERO_HOLD_MS = 720; // 650..800ms hero settle hold
...
function buildNumeral(
  ...
  // Orient: face normal -> +Z (toward viewer in face-local frame), face up -> +Y.
  ...
  // Center the glyph on the recessed face center, sunk slightly into the pocket.
  geo.computeBoundingBox();
  const box = geo.boundingBox!;
  const center = new THREE.Vector3();
  box.getCenter(center);
  geo.translate(-center.x, -center.y, -center.z);
  // Lift so the glyph sits flush at the recessed face surface (engraved/inlaid).
...
export default function TrpgArtisanDiceScene({
  ...
  useEffect(() => {
    ...
    // --- Die geometry: single beveled mesh + engraved gold numerals ---
    ...
    // Numerals attach once the font loads.
    let numerals: THREE.Mesh[] = [];
    ...
    // Cannon world (presentation only; does not decide the result).
    ...
    // Sphere approximation for stable, believable tumble. Result face is set by endQuat.
    dieBody.addShape(new CANNON.Sphere(radius));
    ...
    dieBody.material = new CANNON.Material({ friction: 0.4, restitution: 0.32 });
    floorBody.material = new CANNON.Material({ friction: 0.5, restitution: 0.3 });
    ...
    const tick = (now: number) => {
      ...
      // Phase 3: subtle hero camera settle in the last ~200ms.
