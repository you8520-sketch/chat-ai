...
export default function StatusWidgetChatSettings({
  ...
  const [creatorOn, setCreatorOn] = useState(initialToggles.creatorOn);

  const [userOn, setUserOn] = useState(initialToggles.userOn);

  const [userWidget, setUserWidget] = useState<StatusWidget>(() =>

    parseStatusWidgetJson(initialUserWidgetJson) ?? characterStatusWidgetOrDefault(characterWidgetJson)
  ...
  const mode = useMemo(
    ...
    () => statusWidgetModeFromToggles(creatorOn, userOn),
  ...
  useEffect(() => {
    ...
    setCreatorOn(toggles.creatorOn);
  ...
  return (
    ...
    <section className="space-y-3 text-xs">
      ...
      <div>
        ...
        <p className="mt-2 text-[10px] text-violet-300/90">{formatWidgetBudgetHint(widgetReservedChars)}</p>
      ...
      <div className="space-y-2">
        ...
        <ToggleRow
          ...
          hint={hasCharacterWidget ? "캐릭터에 설정된 상태창을 표시합니다." : "이 캐릭터에 제작자 상태창이 없습니다."}

          checked={creatorOn}

          disabled={!hasCharacterWidget}

          onChange={setCreatorOn}
