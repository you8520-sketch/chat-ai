...
function isExactOrDatedModelId(served: string, acceptedBase: string): boolean {
  ...
  const escaped = normalizedBase.replace(/[.*+?^${}()|[\]\\]/g, "\\/** Served snapshot may carry a dated pin (e.g. `baai/bge-m3-20251117`). */
function isServedModelOf(served: string, requested: string): boolean {
  return served === requested || new RegExp(`^${requested.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}-\\d{8}$`).test(served);
}");
  return new RegExp(`^${escaped}-\\d{8}import type Database from "better-sqlite3";
import {
  buildOpenRouterHeaders,
  OPENROUTER_BASE_URL,
  resolveOpenRouterApiKey,
