...
describe("status meta previous-turn generation continuity", () => {
  ...
  function insertRawStatusMetaMessage(input: {
    id: number;
    content: string;
    statusMetaRaw: string;
  }) {
    const db = getDb();
    db.prepare(
      `INSERT INTO messages (id, chat_id, role, content, user_message_id, generation_status, alternates, active_variant, model, status_meta)
       VALUES (?, ?, 'assistant', ?, NULL, 'completed', '[]', 0, 'm', ?)`
    ).run(input.id, CHAT_ID, input.content, input.statusMetaRaw);
  }
  ...
  it("S9b — malformed JSON and disabled markers do not throw or hide older valid meta", () => {
    seedChat();
    insertAssistantMessage({
      id: MSG_A,
      content: "turn with real status meta",
    ...
    insertRawStatusMetaMessage({
      id: MSG_B,
      content: "malformed status meta row",
      statusMetaRaw: "{not-valid-json",
    });
