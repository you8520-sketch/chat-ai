...
한국 웹소설·캐릭터 중심 소설처럼 현재 초점 인물의 체험에 밀착한다. 관찰만 나열하지 말고 생각·연상·기억·오해·감정·판단이 행동과 자연스럽게 이어지게 한다.

모든 움직임을 순서대로 기록하지 않는다. 분위기·관계·이해·긴장·결과를 바꾸는 디테일만 선택하고 평범한 이동과 생활 동작은 자연스럽게 압축한다.

대사는 이 캐릭터가 지금 이 상대에게 실제로 할 법한 말이어야 한다. 성격과 관계가 말의 내용·생략·농담·망설임·충돌에 드러나게 하며, 독자에게 설정이나 상태를 설명하기 위한 브리핑으로 만들지 않는다.

대사·표정·행동으로 이미 드러난 의미를 “~라는 뜻이었다”, “~의 표시였다”, “~하는 눈빛이었다”, “~에 가까운 어조였다”처럼 다시 판정하지 않는다. 더 이어갈 내용이 필요하면 인물의 구체적인 생각·기억·추론·오해·선택으로 진행한다.

같은 장면의 행동·감각·내면·반응은 인과적으로 연결하고 짧고 긴 문장의 호흡을 자연스럽게 섞는다. 장면을 행동 목록, 신체 부위 목록, 소품 조작 목록, 독립된 정보 조각의 연속처럼 쓰지 않는다.

평온한 장면도 의미 있는 대화·내면·연상·관계 변화·분위기·상황의 결과를 통해 충분히 전개한다. 분량을 미세 행동이나 반복 해설로 채우지 않는다.
