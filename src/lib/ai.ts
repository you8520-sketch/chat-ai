...
items: **유저(${userName})와의 관계에 관련된 물건만** — ① ${userName} 본인의 소지품, ② ${userName}↔상대가 주고받은·나눠 가진·맡긴 물건. **한 사람당 한 줄** — 형식 "이름: 물건1, 물건2, 물건3" (쉼표로 나열). 선물·전달·건넴·양도는 "보낸이→받는이: 물건" 또는 받는 쪽 "이름: 물건"으로. "캐릭터", "유저" 라벨 금지. **절대 금지**: 캐릭터가 원래 갖고 있던 개인 물건, 장면 배경에 놓인 물건, 가구·설비·실내 비품(침대, 의자, 책상, 세면대, 거울 등), 평소 착용 중인 제복·기본 복장. 유저와 무관한 물건은 아무리 자세히 묘사돼도 넣지 마라.
...
honorifics/items/thoughts 규칙은 평소와 동일. "캐릭터","유저" 라벨 금지. items는 **유저(${userName})와의 관계에 관련된 물건만** — ${userName} 본인의 소지품, ${userName}↔상대가 주고받은·나눠 가진·맡긴 물건. 캐릭터가 원래 갖고 있던 개인 물건·배경 물건·가구·설비·착용 중인 제복은 절대 금지.
${RELATIONSHIP_THOUGHT_EXTRACT_RULES.replace(/캐릭터이름/g, charName).replace("유저 내면", `${userName}·유저 내면`)}
