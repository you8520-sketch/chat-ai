...
describe("canonical prose body", () => {
  ...
  it("display formatter repairs pathological sentence-per-paragraph; edit keeps canonical raw", () => {
    const fragmented =
      "렌의 손가락이 방아쇠를 당겼다.\n\n" +
      "두 번째 총성.\n\n" +
    ...
    assert.equal(getDisplayAlignedCanonicalProseBody(fragmented), expectedDisplay);
    assert.equal(
      resolveAssistantEditInitialValue({ content: fragmented }),
      getCanonicalProseBody(fragmented)
    );
    assert.equal(resolveAssistantEditInitialValue({ content: fragmented }), fragmented);
  });

  it("edit entry preserves exact newline structure from DB canonical raw", () => {
    const dbRaw = "AAA\nBBB\n\nCCC";
    assert.equal(resolveAssistantEditInitialValue({ content: dbRaw }), dbRaw);
    assert.equal(normalizeEditedProseForSave(dbRaw), dbRaw);
  });
});
