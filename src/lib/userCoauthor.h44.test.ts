...
describe("H4.4 T1–T10 regressions", () => {
  it("T1 — original H4 이번 턴만 then next turn OFF", () => {
    ...
    assert.equal(turnC.delegation.active, false);
  });
...
describe("H4.4 prompt owners stay mutually exclusive", () => {
  it("ordinary first-turn STANDARD wrapper is unchanged and has no transition sentence", () => {
    ...
    assert.doesNotMatch(wrapped, /POST-DELEGATION RESTORED/);
    ...
    assert.doesNotMatch(owner, /POST-DELEGATION RESTORED/);
  ...
  it("buildContext ordinary STANDARD after turn-only expiry uses STANDARD only, no extra sentence", () => {
    const built = buildContext({
      charName: "테스트_AI_캐릭터",
      chunks: [],
      userNickname: "테스트_유저_캐릭터",
    ...
    assert.doesNotMatch(built.systemPrompt, /POST-DELEGATION RESTORED/);
    const last = built.history[built.history.length - 1]?.content ?? "";
    assert.doesNotMatch(last, /Earlier assistant-authored \[B\] content is scene history only/);
    assert.doesNotMatch(last, /previous user-authoring permission was explicitly limited/);
    assert.doesNotMatch(last, /POST-DELEGATION RESTORED/);
    assert.equal(
