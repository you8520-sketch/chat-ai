...
Main model: Korean RP prose only. Status/HTML/JSON UI is background DeepSeek V4 Flash — do not output status fields, pipe tables, \`\`\`html, or \`\`\`json.`;

/** 제작자 상태창 위젯 ON — V4 Flash가 필드 값 추출, 메인 모델은 RP만 */
...
- DeepSeek V4 Flash (background) extracts widget field values from this turn's prose after generation.`;
