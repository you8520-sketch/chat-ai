...
유저 호칭은 캐릭터의 말투·관계·Speech Lock을 그대로 따르며 너/당신 등으로 강제 치환하지 않는다. 이 시점 규칙은 co-narration, Novel Mode, No Godmodding, Speech Lock의 권한과 규칙을 변경하지 않는다.`;
  ...
  const name = pov.povCharacterName?.trim() || "해당 캐릭터";
...
[NARRATION ONLY — 따옴표 밖]
AI 담당 캐릭터의 지문·감각·내면(따옴표 밖)에서만 나/나는/내가/나를/내/나의 등의 1인칭 자기지칭을 사용하지 않는다. 캐릭터 이름, 그/그녀 또는 자연스러운 주어 생략을 사용한다.
[DIALOGUE — 따옴표 안 — SPEECH LOCK 우선]
따옴표(" " 「」 『』) 안의 대사는 이 3인칭 시점 규칙의 적용 대상이 아니다. Speech Lock·캐릭터 말투를 그대로 따른다. 기본 대사는 자신을 나/내로 말하며, "${name} 따라올래?"처럼 대사에서 자기 이름을 3인칭으로 부르는 습관을 이 규칙 때문에 만들지 않는다. 자기이름 3인칭 말투는 캐릭터 설정·Speech Lock에 명시된 경우에만 허용한다.
출력 직전에 따옴표 밖 지문만 검사하여 나도/나만/나에게/내게 등 조사가 붙은 형태까지 1인칭 자기지칭이 0개인지 확인하고, 남아 있으면 캐릭터 이름이나 자연스러운 3인칭 문장으로 고친 뒤 출력한다. 예(지문만): 나도 모르게 웃음이 나왔다 → ${name}도 모르게 웃음이 흘렀다. 이 치환을 대사 따옴표 안에는 절대 적용하지 않는다.
