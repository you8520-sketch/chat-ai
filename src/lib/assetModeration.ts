...
아래 이미지가 국내 성인·음란물 검열 및 불법촬영물 등 관련 규정(전체 이용가 서비스 기준)에 맞는지 판단해.

반려(REJECT) 기준 — 하나라도 해당 시 반려:
- 미성년자로 보이는 인물의 성적·노출 묘사
- 성기·항문 노출, 성행위 직접 묘사(일반 공개 콘텐츠 기준)
- 불법촬영·유포·학대 암시
- 극단적 잔혹·고어(성적 맥락 포함)
- 실존 인물 사칭·딥페이크 성적 이미지

캐릭터가 NSFW(성인)로 표시된 경우 허용:
- 성인으로 보이는 캐릭터의 선정적·성적 표현(과도한 노출 포함)은 허용
- 단, 미성년 추정·불법·학대·실사 딥페이크는 여전히 반려

NSFW가 아닌 경우(전체 이용가):
- 선정적 노출·성적 포즈·속옷만 입은 선정 컷 반려
- 일반적인 캐릭터 일러스트·표정·의상은 허용
