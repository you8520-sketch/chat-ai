...
/**
 * Legacy sanitizer (pre-fix) — kept for canary comparison logging only.
 * Splits the entire response into sentences/newlines and rejoins every kept
 * sentence with "\n\n", inflating paragraph count. NOT used for production
 * output; only referenced by the hair_sanitizer_paragraph_preserve canary to
 * quantify the legacy inflation.
 */
export function sanitizeHairDescriptionsLegacy(text: string, policy: HairDescriptionPolicy): string {
  if (policy.allowsBeard && policy.allowsBodyHair) return text;
