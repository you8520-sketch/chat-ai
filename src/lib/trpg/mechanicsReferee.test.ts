...
describe("TRPG mechanics referee contract", () => {
  it("reuses the Flash 0731 owner and true-off reply adapter", () => {
    ...
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /\[SCENE\] is PUBLIC resolved fiction/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /tier alone is never enough/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /harm requires directEffect="harm", non-NONE directClass/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /No direct harm requires directEffect="none", directClass="NONE"/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /poison\/venom\/toxin\/bleeding requires periodic_harm/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /paralysis\/neural suppression\/immobilizing control requires control/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /Treatment FAILURE\/SEVERE_FAILURE\/CRITICAL_FAILURE/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /PARTIAL_SUCCESS may reduce but should not full-remove/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /No consumed item: consumeItem must be JSON null/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /"directEffect":"harm","directClass":"LIGHT","cause":"enemy_counter"/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /"kind":"periodic_harm"/);
    assert.match(TRPG_MECHANICS_REFEREE_SYSTEM, /"reason":"treatment failed"/);
