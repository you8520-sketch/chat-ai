...
function isQuotedSpeechBody(body: string): boolean {
...
export function isTrpgSpeakerPrefix(name: string, rest: string, knownNames: readonly string[] = []): boolean {
  ...
  if (!isQuotedSpeechBody(body)) return false;
 ...
 * Only an explicit `이름: "대사"` line becomes a labeled speaker.
 * The UI must not infer speakers from grammar, similes, memory, or the previous line.
 * A trailing `GM:` aside is one table-talk beat.
 */
