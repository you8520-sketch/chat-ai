...
/**
 * Public/new-generation vocabulary. Scenario authors pick a subset.
 * World-only campaigns still use the default 6.
 */
export const TRPG_CANONICAL_STAT_KEYS = [
  "str",
  "dex",
  "con",
  "int",
...
export const TRPG_LEGACY_STAT_KEYS = [
  "app",
  "edu",
  "siz",
  "com",
...
/** Read-only defs for already stored campaign/scenario sheets. Not shown in creator UI. */
export const TRPG_LEGACY_STAT_COMPAT_CATALOG: TrpgStatCatalogEntry[] = [
...
function compatCatalog(): readonly TrpgStatCatalogEntry[] {
  return [...TRPG_STAT_CATALOG, ...TRPG_LEGACY_STAT_COMPAT_CATALOG];
}
...
export function catalogEntry(key: string): TrpgStatCatalogEntry | undefined {
  return TRPG_STAT_CATALOG.find((row) => row.key === key) ?? TRPG_LEGACY_STAT_COMPAT_CATALOG.find((row) => row.key === key);
}

export function isCanonicalStatKey(key: string): boolean {
...
/** Load/play stored campaign or scenario keys, including legacy compat. */
export function parseStatKeys(raw: unknown): string[] {
  return parseStatKeysFromCatalog(raw, compatCatalog());
}
