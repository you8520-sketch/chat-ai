/**
 * Phase G10-SD1 / G10-SD2 — Scene Pacing Controller (Motion Budget).
 *
 ...
 *
 * G10-SD2: Arm Q REPLACES exact SCENE_FLOW_BLOCK with compact [SCENE PACING]
 * so pacing SoT count = 1 (no competing SCENE FLOW owner). Cue wording unchanged.
 */
...
export type ScenePacingArm = "A" | "P" | "Q";
...
export function countPacingOwners(systemText: string): {
  ...
  return {
    ...
    scene_flow,
    genre_scene_mode,
    // Competing owners: SCENE PACING + SCENE FLOW (genre SCENE MODE stripped separately)
    pacing_sot_count: scene_pacing + scene_flow,
  };
}
 ...
 * - A: production (no cue)
 * - P: G10-SD1 — INSERT cue before SCENE FLOW (two pacing owners)
 * - Q: G10-SD2 — REPLACE exact SCENE_FLOW_BLOCK with cue (single pacing owner)
 */
export function applyScenePacingArmToMessages(input: {
  ...
  for (const m of messages) {
    ...
    if (m.content.includes("[SCENE PACING]")) continue;

    if (input.arm === "Q") {
      // Sole change vs G10-SD1 P: REPLACE SCENE FLOW, do not leave it alongside.
