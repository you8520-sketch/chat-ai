/**
 * Phase G10-SD* / G10-D1 / G10-D2 — Scene Pacing + Dialogue Budget experiments.
 *
 ...
 * G10-SD2 Arm Q: REPLACE SCENE_FLOW_BLOCK with [SCENE PACING] (SoT=1).
 * G10-D1 Arm T: system [대화 운용] soft cap (sealed FAIL — soft cue unbound).
 * G10-D2 Arm U sole variable: Q + CURRENT USER TURN terminal dialogue budget
 * (system dialogue-cap owner = 0; private runtime line at user-turn end).
 */
...
export type ScenePacingArm = "A" | "P" | "Q" | "R" | "T" | "U";
...
/**
 * G10-D2 — private runtime terminal dialogue budget (CURRENT USER TURN end).
 ...
 * owner / layout). single_primary only — sim/party/ensemble skip.
 */
export function appendTerminalDialogueBudgetToUserTurn(input: {
  userContent: string;
  decision: ScenePacingDecision;
}): { userContent: string; appended: boolean; skippedReason: string | null } {
 ...
 * - U: G10-D2 — Q + CURRENT USER TURN terminal [이번 응답 대화] (no system cap)
 */
export function applyScenePacingArmToMessages(input: {
  ...
  // T/U share Q pacing path; R keeps state envelope.
  const pacingArm: ScenePacingArm =
    input.arm === "T" || input.arm === "U" ? "Q" : input.arm;
  const cue =
    pacingArm === "R"
  ...
  // G10-D2: append private budget after production terminal length/layout lines.
  if (input.arm === "U") {
