/**
 * Muse Prose M1.1 — admin-only canary gate (default OFF, fail-closed).
 *
 * Requires ALL of:
 *   PROSE_MUSE_M11_ENABLED=1 (or "true")
 *   AND requesting userId is a valid strict positive integer in PROSE_MUSE_M11_USER_IDS
 *   AND modelId is exactly the canonical Muse Spark model ID (meta/muse-spark-1.1)
 *   AND optional PROSE_MUSE_M11_MODEL_IDS, when set, contains the exact canonical ID.
 *
 * No public rollout path for M1.1 in this candidate.
 * No model-specific prose wording in gate logic.
 */

import { isMuseSparkModel, MUSE_SPARK_MODEL_ID } from "@/lib/proseMuseM1Policy";

const ENV_ENABLED = "PROSE_MUSE_M11_ENABLED";
const ENV_USER_IDS = "PROSE_MUSE_M11_USER_IDS";
const ENV_MODEL_IDS = "PROSE_MUSE_M11_MODEL_IDS";
 ...
 * Optional PROSE_MUSE_M11_MODEL_IDS further restricts to exact canonical IDs.
 */
export function isMuseM11EnabledForUser(
...
export const PROSE_MUSE_M11_ENV = {
  ENABLED: ENV_ENABLED,
