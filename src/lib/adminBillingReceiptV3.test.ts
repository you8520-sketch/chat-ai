...
describe("adminBillingReceiptV3", () => {
  it("A — main only, async families proven skipped", () => {
    ...
    assert.equal(receipt.wholeTurn.syncProvablyNone, true);
  ...
  it("K/L — sync_post_turn ledger excluded from async total", () => {
    ...
    assert.ok(Math.abs(receipt.async.knownActualCostUsd - 0.001) < 1e-9);
  });

  it("N — unexpected funding class fails closed", () => {
    const db = createLedgerDb();
    db.prepare(
      `INSERT INTO api_cost_ledger
       ...
       VALUES (?, 1, 60, 'suggested_replies_repair', 'user_funded', 'async_post_turn',
         1, 'cheaperinference', 'flash', 'cheaperinference', 'flash', 'test',
         'settled', 1500, 1.5, 0, 0.001, 'cheaper_inference_billed', datetime('now'))`
    ).run(randomUUID());
    const rows = db.prepare("SELECT * FROM api_cost_ledger WHERE assistant_message_id=?").all(60);
    const receipt = buildAdminBillingReceiptV3({
      usage: baseUsage(),
