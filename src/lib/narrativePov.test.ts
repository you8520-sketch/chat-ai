...
describe("narrative POV owner", () => {
  ...
  it("explicitly switches third-person prose even after first-person history", () => {
    ...
    assert.match(prompt, /지문·감각·내면\(따옴표 밖\)에서만/);
    assert.match(prompt, /나\/나는\/내가\/나를\/내\/나의 등의 1인칭 자기지칭을 사용하지 않는다/);
    assert.match(prompt, /DIALOGUE — 따옴표 안 — SPEECH LOCK/);
    assert.match(prompt, /자기 이름을 3인칭으로 부르는 습관을 이 규칙 때문에 만들지 않는다/);
    assert.match(prompt, /따옴표 밖 지문만 검사/);
    assert.match(prompt, /예\(지문만\): 나도 모르게 웃음이 나왔다 → 권태현도 모르게/);
    assert.match(prompt, /이 치환을 대사 따옴표 안에는 절대 적용하지 않는다/);
  });
});
